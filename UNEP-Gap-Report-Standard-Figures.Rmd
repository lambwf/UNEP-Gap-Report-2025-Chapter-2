...
plot <- data_uncertainties %>% ggplot(.,aes(x=value,y=gas,fill=gas,label=label)) +
  ...
  scale_y_discrete(
    labels = c(
      ...
      expression(N[2]*O),
      expression(CH[4]),
      expression(CO[2]~"LULUCF"),
      expression(CO[2]~"Fossil"),
      expression("Total")
    )) +
...
data_sectors <- data_edgar %>% 
  ...
  summarise(value=sum(value,na.rm=T)/1e9) #%>% 
  #mutate(sector_lv2 = recode(sector_lv2,"Energy | Buildings & other" = "Energy | Buildings and other"))

#colours_9_sectors <- c(colours_9_sectors,"Energy | Buildings and other"="#74a6d4ff","LULUCF"="#7dd396ff")
...
data_sectors <- rbind(data_sectors,data_sectors_luc) %>% 
  filter(year>=1990) # %>%
#  mutate(sector_lv2 = recode(sector_lv2,"Land use change (LULUCF)" = "LULUCF"))
...
data_luc_inv <- read.csv("sources/timeseries_NGHGI_3.0.csv") %>% 
...
data_top_shares <- data_top_shares %>% 
  ...
  mutate(group=ifelse(country %in% c(high_emitters,"Least Developed Countries","Russian Federation","United States of America"),country,"Rest of World")) %>% 
  group_by(group,year) %>% 
  summarise(value=sum(value,na.rm=TRUE))


colours_top6 <- c("Russian Federation" = "#ff9055ff", 
                  "United States of America" = "#659cccff",
                  "China" = "#e5b82cff",
                  "European Union" = "#17becfff",
                  "Brazil" = "#7dd396ff",
                  "Indonesia" = "#7dd396ff",
...
high_emitters <- c(high_emitters,"European Union", "World","Russian Federation","United States of America")
...
p2 <- labels %>% 
  ...
  geom_text_repel(
    ...
    lineheight=0.9) +
  annotation_custom(grob = textGrob(paste0("Share in ",max_year-1,":"),
                                    x = unit(0.03, "npc"), y = unit(0.95, "npc"),hjust = 0, vjust = 0,
                                    gp = gpar(col = "#636363", fontsize = 10, lineheight=0.9)),
                    xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf) +
  scale_colour_manual(values=c(colours_top6,"Rest of world" = "#818181ff","Least Developed Countries"="#9467bdff",
                               "United Kingdom"="#d45087ff")) +
...
### Contribution to total annual change, with G20
...
data_contribution <- data_contribution %>% 
  left_join(.,countries) %>% 
  left_join(.,cc_G20) %>% 
  mutate(iso=ifelse(is.na(TOP) & G20==1,"RESTG20",iso)) %>% 
  mutate(iso=ifelse(is.na(TOP) & is.na(G20),"REST",iso)) %>% 
  mutate(country=ifelse(iso=="REST","Rest of World",country)) %>%
  mutate(country=ifelse(iso=="RESTG20","Rest of G20",country)) %>% 
  group_by(iso,country,year) %>% 
  summarise(value=sum(value,na.rm=T))
...
ggsave(plot,filename = "UNEP-EGR-2025-top-G20-contribution-annual.png",path = "results/standard/",device = "png",height = 4,width=8,dpi=300)
ggsave(plot,filename = "UNEP-EGR-2025-top-G20-contribution-annual.pdf",path = "results/standard/",device = cairo_pdf,height = 4,width=8)
ggsave(plot,filename = "UNEP-EGR-2025-top-G20-contribution-annual.svg",path = "results/standard/",device = 'svg',height = 4,width=8)
...
saveWorkbook(wb,paste0("results/standard/UNEP-EGR-2025-top-G20-contribution-annual.xlsx"),overwrite=T)
...
data_countries <- data_countries %>% 
  mutate(country=ifelse(iso=="RUS","Russian Federation",country)) %>% 
  mutate(country=ifelse(iso=="USA","United States of America",country)) %>% 
  mutate(country=ifelse(iso=="KOR","Republic of Korea",country))
